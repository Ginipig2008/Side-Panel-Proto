...
import { Search, X, Activity, ChevronLeft, Check } from 'lucide-react';
import { actionData, ActionDataNode } from '../data/actionData';
...
const ActionPanel: React.FC<ActionPanelProps> = ({ onClose, onAddClip, onReplaceClip, panelMode = 'default', setPanelMode, targetClip, setTargetClip, pendingClip, setPendingClip, selectedClip, setSelectedClip, onCancelPreview, onApplyPreview, handleAddDialogueClip, allDialogueClips }) => {
    const [actionPath, setActionPath] = useState<ActionDataNode[]>([]);

    const handleBack = () => {
        setActionPath(prev => prev.slice(0, -1));
    ...
    useEffect(() => {
        if (panelMode === 'replace' && targetClip?.name) {
            let foundPath: ActionDataNode[] = [];

            const findNodePath = (nodes: ActionDataNode[], currentPath: ActionDataNode[]): boolean => {
    ...
    const currentFolder = actionPath.length > 0 ? actionPath[actionPath.length - 1] : null;
                ...
                <>
                    ...
                    <div className="w-full p-4 rounded-xl bg-gradient-to-r from-[#7C5CFC] to-[#6A4DF0] text-white shadow-lg relative overflow-hidden group cursor-pointer transition-transform duration-200 hover:scale-[1.02]">
                        <div className="relative z-10">
                            {panelMode === 'replace' ? (
                                <>
                                    <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-white/20 mb-2 backdrop-blur-sm shadow-sm ring-1 ring-white/30">
                                        REPLACE MODE
                                    </span>
                                    <h3 className="font-semibold text-lg leading-tight drop-shadow-sm">Replace Window</h3>
                                    <p className="text-white/90 text-xs mt-1 drop-shadow-sm">Select an action to replace "{targetClip?.name}"</p>
                                </>
                            ) : (
                                <>
                                    ...
                                    <h3 className="font-semibold text-lg leading-tight">Create your own motion</h3>
                                    <p className="text-white/80 text-xs mt-1">AI-powered motion generation</p>
                                </>
                            )}
                        </div>
                    ...
                    <div>
                        <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
                            {actionPath.length === 0 ? 'Categories' : actionPath.length === 1 ? 'Subcategories' : 'Clips'}
                        </h3>
                        <div className="grid grid-cols-2 gap-3">
                            {(currentFolder ? currentFolder.children : actionData)?.map((item: ActionDataNode) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className={`absolute inset-0 transition-colors duration-200 ${isLeaf ? 'bg-[#7C5CFC]/5 group-hover:bg-[#7C5CFC]/10' : 'bg-gray-50 group-hover:bg-gray-100'}`} />

                                        {/* Content */}
                                        <div className="relative z-10 h-full flex flex-col justify-end p-3">
                                            <span className={`font-medium text-sm transition-colors duration-200 line-clamp-2 ${isLeaf ? 'text-[#7C5CFC] group-hover:text-[#6A4DF0]' : 'text-gray-800 flex items-center justify-between'}`}>
                                                {item.name}
                                                {!isLeaf && <ChevronLeft size={14} className="text-gray-400 rotate-180" />}
                                            </span>
                                            {!isLeaf && (
                                                <span className="text-[10px] text-gray-500 transition-colors duration-200 mt-0.5">
